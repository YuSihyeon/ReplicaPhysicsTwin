...
첨부 원본 `replica_physics_demo_reverse_realtime.mp4`는 2560 × 1368, 14 FPS, 97 frames, 약 6.93초다. 샘플 프레임에서 자전거의 이동과 의류 변형을 확인했다. 파일명에 `reverse`가 있어 순방향 물리 시간·충격량·실시간 실행 속도를 이 영상만으로 입증할 수 없다. 원본을 재편집하거나 재생 방향을 임의로 해석하지 않고 갤러리에 해당 제한을 명시했다.
